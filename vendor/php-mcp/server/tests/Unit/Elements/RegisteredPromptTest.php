...
dataset('prompt_format_errors', [
    ...
    'invalid_content_structure_in_array' => ['promptReturnsArrayWithInvalidContentStructure', "/Invalid message format at index 0. Expected an array with 'role' and 'content' keys./"], 
...
it('can be serialized with ListCompletionProvider instances', function () {
    ...
    $array = $original->toArray();
    expect($array['completionProviders']['status'])->toBeString(); 
...
it('can be serialized with EnumCompletionProvider instances', function () {
    ...
    $array = $original->toArray();
    expect($array['completionProviders']['priority'])->toBeString(); 
