...
it('can handle tool call request and return result', function () {
    ...
    $this->schemaValidator->shouldReceive('validateAgainstJsonSchema')->with($args, $toolSchema->inputSchema)->andReturn([]); 
...
it('can handle prompts list request and return paginated prompts', function () {
    ...
    expect($resultP2->prompts)->toHaveCount(DISPATCHER_PAGINATION_LIMIT); 
...
it('can handle prompt get request and throw exception if required argument is missing', function () {
    ...
    $request = GetPromptRequest::make(1, $promptName, ['other_arg' => 'value']); 
