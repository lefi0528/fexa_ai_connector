...
class ResourceHandlerFixture
{
    ...
    public function returnStream(string $uri) 
    {
    ...
    public function returnMixedArrayWithResourceTypes(string $uri): array
    {
        return [
            "A raw string piece", 
            TextResourceContents::make($uri . "_rc1", 'text/markdown', '**Markdown!**'), 
            ['nested_array_data' => 'value', 'for_uri' => $uri], 
            EmbeddedResource::make(TextResourceContents::make($uri . "_emb1", 'text/csv', 'col1,col2')), 
        ];
    }
